...
class Everzet_Jade_Lexer_Lexer implements Everzet_Jade_Lexer_LexerInterface
{
    ...
    public function __construct($indentSize = 4)
    {
    ...
    protected function scanCode()
    {
        ...
        if (preg_match('/^(!?=|-)([^\n]+)/', $this->input, $matches)) {
            ...
            $token->unescaped = (isset($flags[0]) && '!' === $flags[0]);
...
//            $input      = substr($this->input, 1, $index - 1);
    ...
    protected function scanIndentation()
    {
        ...
        if (preg_match('/^\n( *)/', $this->input, $matches)) {
            ...
            $indents    = mb_strlen($token->value) / $this->indentSize;

            if (0 != mb_strlen($token->value) % $this->indentSize)
            {
                $message = 'Line #'.$this->lineno.': Invalid indentation found. Got %.2f , %d unnecessary spaces';

                if (mb_strlen($token->value) < $this->indentSize * ($this->lastIndents + 1))
                {
                    $message = 'Line #'.$this->lineno.': Invalid indentation found. Got %.2f , need %d more spaces';
                }

                throw new Everzet_Jade_Exception(sprintf(
                    $message
                    , $indents
                    , abs(mb_strlen($token->value) - $this->indentSize * ($this->lastIndents + 1))
                ));
            }
            ...
            if (mb_strlen($this->input) && "\n" === $this->input[0]) {
                ...
                throw new Everzet_Jade_Exception(sprintf(
                    'Line #'.$this->lineno.': Invalid indentation found. Spaces count must be a multiple of two, but %d got.'
                ...
                throw new Everzet_Jade_Exception(sprintf(
                    'Line #'.$this->lineno.': Invalid indentation found. Got %.2f, but expected %d , %d unnecessary spaces'
                  , $indents
                  , $this->lastIndents + 1
                  , mb_strlen($token->value) - $this->indentSize * ($this->lastIndents + 1)
                ));
            } elseif ($this->lastIndents > $indents) {
    ...
    protected function getDelimitersIndex($begin, $end)
    {
        ...
        for ($i = 0, $length = mb_strlen($string); $i < $length; ++$i) {
            ...
            */
            ...
            /**
             * 对于多字节字符串，通过数组下标方式访问不能获取正确位置的字符
             */
            ...
            if (!$sbrac && !$dbrac && $begin === $char) {
                ++$nbegin;
            } elseif (!$sbrac && !$dbrac && $end === $char) {
                if (++$nend === $nbegin) {
                    $position = $i;
                    break;
                }
