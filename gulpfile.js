...
var front_hostname = 'js.catfood.wap.grid.sina.com.cn';
var back_base = 'server_back'; //模拟后端的文件放置目录
var back_hostname = 'catfood.codezz.cn'; //后端的HOST，目的是真实模拟后端的页面路由请求，提供出前端可仿真的功能，比如 /index 对应 /html/index.html
...
gulp.task('build', function() {
    del(['build/*'], function() {
        gulp.src(['src/css/**/*.*']).pipe($.steelCssFix(cssfix_option)).pipe($.minifyCss({
            compatibility: 'ie8'
        }))
...
function dealCss(isDebug) {
    if (isDebug) {
        gulp.src(['src/css/**/*.*']).pipe($.steelCssFix(cssfix_option)).pipe(gulp.dest(front_base + '/css/'));
    } else {
        gulp.src(['src/css/**/*.*'])
            .pipe($.steelCssFix(cssfix_option))
            .pipe($.minifyCss({
                compatibility: 'ie8'
            }))
