...
class Comm_Db
{
    static protected $defaultConfigName = 'db_pool';
    static protected $dbs = array();
    
    /**
     * 根据配置文件自动配置数据库池
     * 
     * @param string $configName [可选] 配置名。若为空，则使用 db_pool 作为默认配置名。该配置由Comm_Config加载
     ...
     * 初始化DB对象
     * 
     * @param string $dbAlias 数据库连接对象别名。该别名在配置文件中定义。
     * 
     * @return object
     */
    ...
    public static function pool($dbAlias)
    {
        return self::connect($dbAlias);
    }
