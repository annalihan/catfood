...
class Everzet_Jade_Parser
{
    ...
    protected function expectTokenType($type)
    {
        if ($type === $this->lexer->predictToken()->type) {
            ...
            throw new Everzet_Jade_Exception(sprintf('Line #'.$this->lexer->getCurrentLine().': Expected %s, but got %s, line %d', $type,
    ...
    protected function parseCode()
    {
        ...
        $node   = new Everzet_Jade_Node_CodeNode($token->value, $token->buffer, $this->lexer->getCurrentLine(), $token->unescaped);
    ...
    protected function parseFilter()
    {
        ...
        $type = $this->lexer->predictToken(2)->type;
    ...
    protected function parseTag()
    {
        ...
        while (true) {
            $tokenType = $this->lexer->predictToken()->type;
            switch ($tokenType) {
                case 'id':
                case 'class':
                    $token = $this->lexer->getAdvancedToken();
        ...
        //.的处理
        /*if ($this->lexer->predictToken()->value == '.')
        {
            $textOnly = true;
            $this->lexer->getAdvancedToken();
        }*/
        ...
        if ('indent' === $this->lexer->predictToken()->type)
        {
            if ($textOnly)
            {
                ...
                $node->addChild($this->parseTextBlock());
                //$node->addChild($this->parseText());
            }
            else
            {
                $node->addChild($this->parseBlock());
